...
| 🕒 **时间过滤** | `advanced_search` 支持 `timeRange`（day/week/month/year、`12h`、`3d`、绝对日期） |
...
# 或从 GitHub 安装（插件在仓库子目录 dsh-thin-search/ 下）
dsh plugin --profile web add "github:loommii/dsh-thin-search#path:dsh-thin-search"

# 或本地源码（开发/自用）
git clone git@github.com:loommii/dsh-thin-search.git
dsh plugin --profile web add /path/to/dsh-thin-search/dsh-thin-search
```
...
![设置页](https://raw.githubusercontent.com/loommii/dsh-thin-search/main/dsh-thin-search/assets/settings-free.png)
...
#### advanced_search（时间过滤）

让 agent 搜"最近一周的新闻"、"最近 3 天的消息"，它会调用 `advanced_search` 带 `timeRange` 参数，同样走自动回退链。
...
支持过滤的引擎（SearXNG / DDG / DDG Lite）会把自定义天数映射到最近似档位（`≤2 天 → day`，`≤14 天 → week`，`≤90 天 → month`，否则 `year`）；Bing / AnySearch 无对应参数会忽略。带 `timeRange` 时支持过滤的引擎自动排到回退链前面。
...
- **Time filtering** — `advanced_search` with `timeRange`
...
dsh plugin --profile web add "github:loommii/dsh-thin-search#path:dsh-thin-search"   # from GitHub (monorepo subdir)
```
