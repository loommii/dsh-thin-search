...
const THIN_SEARCH_NS = settingsNamespace("thin-search");
...
function apply(ctx, config) {
  ...
  // 搜索增强：用 LLM 把口语化输入规范化为搜索关键词（仅 searchEnhance 开启时调用，消耗 token）
  const ENHANCE_SYSTEM_PROMPT = [
    "你是搜索关键词优化器。把用户的口语化搜索意图转换为最适合搜索引擎的关键词查询。",
    "规则：",
    "1. 去掉口语语气词（今天、多少、怎么、如何、是什么、帮我、我想知道、请问 等）",
    "2. 保留核心实体与意图（货币对、地名、产品名、问题主题 等）",
    "3. 关键词用空格分隔，不超过 8 个词，不要标点",
    "4. 只输出一行关键词，不要解释，不要引号",
    "5. 如果输入已经是规范关键词，原样输出",
    "示例：'今天美元兑换日元的汇率多少？' → '美元 日元 汇率'",
    "示例：'北京明天天气怎么样' → '北京 明天 天气'",
    "示例：'帮我找一下 DeepSeek 的最新版本发布说明' → 'DeepSeek 最新版本 发布说明'",
  ].join("\n");
  const ENHANCE_PROMPT = "输入：{query}";
  ...
  async function enhanceQuery(query, signal) {
    ...
    try {
      ...
      for (const candidate of chain) {
        ...
        const attempt = await enhanceWithModel(llm, candidate, ENHANCE_SYSTEM_PROMPT, ENHANCE_PROMPT, query, controller);
        if (attempt.ok) {
  ...
  installSettingsSection(ctx, THIN_SEARCH_NS, Config, config ?? {}, {
    setSource: (source) => {
      applySource(source);
    },
    onChange: () => {
