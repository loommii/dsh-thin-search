...
// 自定义设置页/命令使用的逻辑 namespace；DSH 0.1.7 的真实 Settings 表单
// 以 cordis.patch.yml 中的 profile entry id 标识，因此桥接层负责映射两者。
const THIN_SEARCH_NS = "thin-search";
// 沿用旧版 settings.yaml 的 section 名，让 DSH 0.1.7 首次启动时可把
// legacy thin-search: 自动导入到同名 profile entry。
const SETTINGS_ENTRY_ID = THIN_SEARCH_NS;
...
// DSH 0.1.7 把 `.volatile()` 配置字段作为稳定引用传入插件；旧版加载器
// （以及测试桩）仍可能传普通值，因此两种形态都接受。
...
function readConfigSnapshot(config) {
  const source = config ?? {};
  return Object.fromEntries(CONFIG_FIELDS.map((key) => [key, readConfigValue(source[key])]));
}
...
/**
 * 无 Settings 服务上下文时的兼容回退：优先 DSH_PROFILE，否则枚举
 * $DSH_HOME/profiles/* 找包含本插件的 profile。正常 DSH 0.1.7 路径由
 * SettingsForms.documentPath 提供，不依赖环境变量或目录扫描。
 */
...
async function readProfilePatchFile(file = resolveProfilePatchPath()) {
  try {
...
async function detectEngineMode(file = resolveProfilePatchPath()) {
  const content = await readProfilePatchFile(file);
  return findManagedBlock(content) === null ? "thin" : "official";
}

/**
 * 切换模式：official 时写入托管块，thin 时删除托管块。
 * 使用 DSH ConfigEditor 相同的 package.json 文件锁与原子写，避免和
 * SettingsForms.mutate 并发时覆盖彼此；文件里其他 patch/注释原样保留。
 */
async function setEngineMode(mode, file = resolveProfilePatchPath()) {
  return withFileLock(join(dirname(file), "package.json"), async () => {
    let content = await readProfilePatchFile(file);
...
// 判定一次 settings 写入失败是否为"版本冲突"。
// 优先用核心声明的稳定机器码 `code`（SettingsConflictError.code === "SETTINGS_CONFLICT"），
// instanceof 仅作兜底：当插件解析到的 @deepseek-ai/dsh-settings 与运行时的不是同一份副本时
// （例如插件自带 node_modules 或 profile 回退链接指向不同版本），类身份不同会让 instanceof 失效。
function isSettingsConflict(error) {
  if (error === null || typeof error !== "object") return false;
  if (error.code === "SETTINGS_CONFLICT") return true;
  return typeof SettingsConflictError === "function" && error instanceof SettingsConflictError;
}

function makeBridgeRoutes(settings, search, testEngine, readWebProviderId, enhance, llm, settingsEntryId = SETTINGS_ENTRY_ID) {
  const currentWebProviderId = () =>
    typeof readWebProviderId === "function" ? readWebProviderId() : readWebProviderId;
  const allowlisted = () =>
    settings
      .describe({ redactSecrets: true })
      .filter((descriptor) => String(descriptor.ns) === settingsEntryId)
  ...
  const handlers = {
      ...
      return {
        ...
        value: {
          namespaces: allowlisted()
            ...
            .map((descriptor) => ({ ...toView(descriptor), ns: THIN_SEARCH_NS })),
    ...
    async mutate(request) {
      ...
      const { ns } = body;
      if (ns !== THIN_SEARCH_NS || !allowlisted().includes(settingsEntryId)) {
        return { ok: false, code: "settings-not-exposed", message: `settings namespace "${ns}" is not exposed` };
      }
      const expectedRevision = typeof body.expectedRevision === "number" ? body.expectedRevision : undefined;
      try {
        await settings.mutate(settingsEntryId, body.ops, expectedRevision);
      } catch (error) {
        // 判定用 `code` 而非 instanceof：dsh 核心包在 profile 安装树中可能有多份副本
        // （插件自带 node_modules / profile 回退链接），跨副本的类身份不一致会让
        // instanceof 静默失败。`code` 是核心声明的稳定机器码，专供这种线上协议映射。
      ...
      const descriptor = settings.describe({ redactSecrets: true }).find((candidate) => String(candidate.ns) === settingsEntryId);
      if (descriptor === undefined) {
        return { ok: false, code: "internal", message: `settings entry "${settingsEntryId}" was disposed after the mutate` };
      }
      return { ok: true, value: { ...toView(descriptor), ns: THIN_SEARCH_NS } };
    },
    async engineMode(request) {
      ...
      try {
        const documentPath = typeof settings.documentPath === "string"
          ? settings.documentPath
          : resolveProfilePatchPath();
        if (action === "set") await setEngineMode(mode, documentPath);
        const current = await detectEngineMode(documentPath);
        return {
          ok: true,
          value: {
            mode: current,
            // 运行时 web.searchProviderId 是否已指向当前模式（false = 需要重启才生效）
            applied: currentWebProviderId() === (current === "official" ? OFFICIAL_PROVIDER_ID : PROVIDER_ID),
          },
        };
      } catch (error) {
...
const Config = z.object({
  provider: z.string().default("anysearch").volatile(),
  searchEnhance: z.boolean().default(false).volatile(), // 搜索增强：用 LLM 把口语化输入规范化为搜索关键词（消耗 token）
  enhanceModel: z.string().default("").volatile(), // 增强模型（"provider:model"；空 = 跟随 DSH 默认模型）
  enhanceSystemPrompt: z.string().default("").volatile(), // 自定义搜索增强系统提示词（空 = 使用内置默认）
  proxy: z.string().default("").volatile(), // HTTP/HTTPS 代理（如 http://127.0.0.1:7890；空 = 直连；DDG/SearXNG/GitHub 等外网引擎需要）
  cache: z.boolean().default(true).volatile(),
  cacheTtl: z.number().default(5).volatile(),
...
function apply(ctx, config) {
  // DSH 0.1.7 的 volatile 字段是稳定引用；每次操作读取一次当前快照。
  const current = () => readConfigSnapshot(config);
  const logger = ctx.logger;
  // Loader Entry.id 是层级全路径；SettingsForms 以 patch 行的 options.id
  // （本插件为 thin-search）作为 namespace。
  const settingsEntryId = ctx.fiber?.entry?.options?.id ?? SETTINGS_ENTRY_ID;
  ...
  // DSH 0.1.7 将设置表单从可动态注册的 namespace 改为 Loader profile entry。
  // 本插件仍提供自定义页面，因此关闭自动页面生成；业务配置仍由 Settings
  // 投影并通过 SettingsForms.mutate 写入 profile 的 cordis.patch.yml。
  ctx.inject(["settings"], (settingsCtx) => {
    settingsCtx.effect(() => {
      if (typeof settingsCtx.settings.configure !== "function") {
        throw new TypeError("dsh-thin-search requires the DSH 0.1.7 SettingsForms API");
      }
      return settingsCtx.settings.configure({ auto: false }, ctx.fiber);
    });
  });

  // volatile 配置提交后由 Loader 派发到所属 fiber；此时各引用已经更新。
  ...
  // dsh-web 的类型把 searchProviderId 标为 readonly；这里只作 best-effort，
  // 正常安装路径由 cordis.patch.yml 的 searchProvider: thin-search 生效。
        ...
        defineTool({
          ...
          output: {
            ...
            render(args, value) {
              ...
              const header = `Search (${value.provider}${args.timeRange ? `, timeRange=${args.timeRange}` : ""}):`;
